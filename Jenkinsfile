        ...
        stage("deployment") {
            steps {
                script {
                    ...
                    sh 'ssh jenkins@65.2.29.148 "cd /var/www/html/team1-fe/ && sudo npm install --legacy-peer-deps"'
                    sh 'ssh jenkins@65.2.29.148 "cd /var/www/html/team1-fe/ && sudo npm run build"'
                }
            }
        }
    }
